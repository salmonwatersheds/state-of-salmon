...
# Update 2025 data for systems available -- Taku + Stikine
tbrck_raw_updated <- tbrck_raw %>%
	rbind(data.frame( # 2024 data
		...
		Rate = rep(NA, 3),
		TotalRun = rep(NA, 3))) %>%
	rbind(data.frame( # 2025 datan  (From data provided by Aaron Foos, not same source as 2024 data)
		Stock = c("Taku", "Stikine"),
		Year = rep(2025, 2),
		Escapement = c(42972, 14813), # 'unadjusted CAD spawning escapement' for Taku, 'Canadian escapement' for Stikine
		RateType = rep(NA, 2),
		Rate = rep(NA, 2),
		TotalRun = rep(NA, 2)))

# Harbest data - from data provided by Aaron Foos on 2026-07-08
tbrck_harvest <- data.frame(Year = rep(c(2024:2025), 3), 
													 Stock = rep(c("Alsek", "Taku", "Stikine"), each=2),
													 CAD_harv = c(40, NA, 22, 44, 49, 263),
													 USA_harv = c(195, NA, 372, 857, 37, 12)) 

# Calculate Total Run for 2024 and 2025
tbrck_raw_updated_24_25 <- tbrck_raw_updated %>% 
	filter(Year %in% 2024:2025) %>%
	left_join(tbrck_harvest, by=c("Year", "Stock")) %>%
	mutate(TotalRun = Escapement + CAD_harv + USA_harv)
	
tbrck_raw_updated$TotalRun[tbrck_raw_updated$Year >= 2024] <- tbrck_raw_updated_24_25$TotalRun
...
points(x=nass_ctc$Year, y=nass_ctc$Esc/1000, col="purple")
points(x=nass_ctc$Year, y=nass_ctc$t.run/1000, col="blue") # See how t.run from ctc (blue) compares to total run from Nisgaa
