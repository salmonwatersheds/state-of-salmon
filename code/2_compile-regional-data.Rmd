...
#### Chinook {.unnumbered}
The only available data for Chinook salmon in Haida Gwaii are from a single enumeration project on the Yakoun River. Although monitoring on the Yakoun has recently been restored, the most recent estimate for Yakoun Chinook that is publicly available is from 2006. We show Yakoun River spawner abundance up to 2006, but no estimates were available for the most recent generation making the current status for Haida Gwaii Chinook "Unknown". 

These estimates also match the estimated escapement to Area 1 in the  @NorthernBoundaryTechnicalCommittee2023USCanadaNorthern (Appendix 30).

```{r haida gwaii chinook, fig.cap="**Above:**Spawner abundance of Haida Gwaii chinook salmon. The raw data is shown by points and the smoothed metric by solid lines. Dashed horizonal lines represent the long-term average."}
...
hgck <- read.csv(here("data/spawner_surveys.csv"), na.strings = c(-989898)) %>%
...
sksh <- read.csv(here("data/SkeenaSteelhead_1956-2024.csv"))
