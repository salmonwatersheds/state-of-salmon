...
```{r transboundary chinook figs, echo = FALSE, include = TRUE, results = 'asis', fig.width = 7, fig.height = 6, fig.cap = "**Above:** Chinook salmon spawner abundance (open points) and total abundance (closed points) showing the breakdwon in the Alsek, Taku, and Stikine river basins."}
...
tbrck_raw_updated %>% ggplot() +
	geom_area(aes(x=Year, y=TotalRun/1000, fill=factor(Stock, levels=c("Stikine", "Taku", "Alsek"))), 
								alpha=0.6) + 
	scale_fill_viridis_d() +
	labs(y="Total abundance (thousands)", fill="") +
	theme_minimal() 
...
Placeholder for description of sockeye data sources?

```{r transboundary sockeye}
...
Monitoring of pink and chum salmon in the Transboundary region is limited, and the best available information is the Canyon Island fish wheel. 

```{r transboundary pink}
...
for(s in 2:5){
	...
	#plot_abund(hg.s_sps) # tabs not working with this included can try
...
<!-- All plots in Nass section - in Rmd but not in final html? -->
...
```{r skeena other species, echo = FALSE, include = TRUE, results = 'asis', fig.width = 7, fig.height = 6, fig.cap = "**Above:** (a) Time series of spawner abundances showing raw sum of counts for indicator streams (teal) and expanded estimates of spawner abudnance to indicator and non-indicator streams (black). This expanded time series is what is used to assess the current state and trends in spawner abundance for the region and species. Alternative time series from the PSC's Northern Boundary Technical Committee Report (Appendix 32) is shown in red. (b) The values of Expansion Factor 1 that accounts for unmonitored indicator streams in a given year, and is calculated based on the proportional contribution of the unmonitored indicator stream(s) to the sum of all indicator streams in a (reference) decade. See section on Expansion Factors below for further details."}
...
plot_expansions(speciesname = species[s],
	...
	#plot_abund(skeena.s_sps) # messes up tabs - figure out how to include?
...
```{r central coast chinook, echo = FALSE, include = TRUE, results = 'asis', fig.width = 7, fig.height = 6, fig.cap = "**Above:** Comparison of Chinook escapements on the Central Coast."}
...
```{r east VI chinook figs, echo = FALSE, include = TRUE, results = 'asis', fig.width = 7, fig.height = 6, fig.cap = "**Above:** Spanwer abundance (open, light) and total abundance (closed, dark) for three CTC indicator stocks and the sum, used to assess EVIMI Chinook."}
