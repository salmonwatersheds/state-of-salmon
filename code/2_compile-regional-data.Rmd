...
# Load data ---
ytck <- read.csv('../data/yukon_chinook_appendixB11.csv', na.strings = "-") 
...
# Load data ---
region_spawners <- sp <- readRDS("../output/expanded-spawners/Haida Gwaii-spawners.rds")
...
# Load Nisgaa data
nass_nisgaa <- read.csv("../data/2025NassStockAssessment_Table20+22.csv")
...
region_spawners <- sp <- readRDS("../output/expanded-spawners/Skeena-spawners.rds")
yrs <- as.numeric(dimnames(sp)[[3]])
...
# TCCHINOOK Table B3
cc_ctc <- readxl::read_xlsx("../data/TCCHINOOK-25-02-Appendix-B-Escapement-Detailed.xlsx", sheet = "B3", 
...
region_spawners <- sp <- readRDS("../output/expanded-spawners/Central Coast-spawners.rds")
yrs <- as.numeric(dimnames(sp)[[3]])
...
region_spawners <- sp <- readRDS("../output/expanded-spawners/East Vancouver Island & Mainland Inlets-spawners.rds")
yrs <- as.numeric(dimnames(sp)[[3]])
...
region_spawners <- sp <- readRDS("../output/expanded-spawners/West Vancouver Island-spawners.rds")
yrs <- as.numeric(dimnames(sp)[[3]])
...
2026 report update: NO INDICATOR STREAMS FOR WVI PINK
...
```{r west VI pink, fig.width = 7, fig.height = 6, fig.cap="**Above:**Spawner abundance of West Vancouver Island pink salmon. The raw data is shown by points and the smoothed metric by solid lines. Dashed horizonal lines represent the long-term average.", eval=FALSE}
## Process data ---
wvi.s <- sp[2, "Pink", ]
# Get rid of NAs 
wvi.s <- wvi.s[1:max(which(!is.na(wvi.s)))]
...
write.csv(sps_data, "../output/sps-data.csv", row.names = FALSE) # Always have most recent # Not until this script is confirmed to be working 
