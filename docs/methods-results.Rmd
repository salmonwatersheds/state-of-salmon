...
The [Pacific Salmon Foundation (PSF)](https://www.psf.ca) is developing an inaugural **State of Pacific Salmon in Canada** publication, which will address these questions and be tailored to the questions of a salmon-informed public audience. The outputs will be updated regularly (e.g. annually or biennially) to reflect new data and changes to the state of Pacific salmon populations.
...
For Chinook, coho, and sockeye, we assessed regional abundance in the Transboundary using estimates of border escapement provided in the Pacific Salmon Commission's [Joint Transboundary Technical Committee Reports](https://www.psc.org/publications/technical-reports/technical-committee-reports/transboundary/). The regional spawner abundance and run size of Chinook was calculated as the sum of escapement and run size, respectively, to the Stikine, Alsek, and Taku watersheds, available from Table B2 of @CTC2023AnnualReportCatch and provided to E. Hertz in Excel format following a data request. Sockeye spawner abundance and run size was the sum escapement or Terminal Run, respectively, from the Stikine River [Appendix B21; @TTC2022FinalEstimatesTransboundary] and the Taku River [Appendix D15; @TTC2022FinalEstimatesTransboundary]. Coho spawner abundance and run size are available for the Taku River only, taken from Appendix D20 of @TTC2022FinalEstimatesTransboundary.
...
1. If the species and region had no data in the most recent generation, it was deemed `Data Deficient` even if historical abundance information was available. This was the case for **Haida Gwaii Chinook**, for which there were no reliable estimates of spawners since 2006.
...
* These trends in aggregated spawner abundance provide a broad overview of how salmon are doing, but must be interpreted with caution due to the diversity of populations with potentially different outlooks within each region and species. For example, Chinook in Vancouver Island & Mainland Inlets have an aggregate spawner abundance that is `r round(sps_metrics$status[sps_metrics$region == "Vancouver Island & Mainland Inlets" & sps_metrics$species == "Chinook" & sps_metrics$type == "Spawners"]*100, 1)`% above historical average, but some stocks (e.g., West Coast Vancouver Island) are of conservation concern. **Preserving the population diversity within regions is key to the long-term resilience of salmon, particularly in the face of climate change.**
* The status of spawner abundance is, in all cases, more positive than the status in run size, highlighting that declines in fisheries have been pervasive. Further, for all species and regions that had data on run size, current abundance is below the long-term historical average.

## All regions

**Highlights**

* Across regions, pink salmon seem to be doing the best (spawner abundance above historical average in 4 of 7 regions assessed) and steelhead are doing the worst (spawner abundance below historical average in all 5 regions assessed).
* The Fraser region has the most positive outlook, with spawner abundances of chum, coho, and pink all above historical average. Although the historically productive fisheries supported by this Region have been decimated, and run size for coho and pink are well below historical average, stable trends over the most recent 3-generations in both spawner abundance and run-size for these species provide hope that declines can be stemmed and populations rebuilt.
* Further north, in the Yukon and Transboundary, almost all species (with the exception of Transboundary pink salmon) are well below historical average with recent declines in either spawners, run size, or both. This finding is concerning, given that climate change may render southern habitats less suitable for salmon into the future and more northern populations must be nurtured as potential strongholds. Management of salmon harvest in these northern regions is complex, and involves international treaties that are not easily negotiated or adapted in response to environmental changes or emerging trends, challenging efforts to stem declines and recover these populations. 


**Below**: Summary of current status relative to historical average (Metric 1) for **spawner abundance**. Regions and species below historical average are in red and above historical average are in green. Black cells indicate species that are not thought to be present in the given region, and grey cells indicate there were no available abundance data for that species and region combination. Solid red cells are species and regions with absolute spawner abundance < 1000, deemed at risk of extirpation regardless of Metric 1 value.
```{r, echo = FALSE, message = FALSE, warning = FALSE}
btn_table.all(sps_metrics[which(sps_metrics$type == "Spawners"), c("region", "species", "status")],
							type = "Spawners")
```

**Below**: Summary of current status relative to historical average (Metric 1) for **run size**. Regions and species below historical average are in red and above historical average are in green. Black cells indicate species that are not thought to be present in the given region, and grey cells indicate there were no available abundance data for that species and region combination. 
```{r, echo = FALSE, message = FALSE, warning = FALSE}
...
* Trends in Chinook run size show statistically significant declines of 3% and 4% **per year** over long and short time frames, respectively, highlighting the significant, persistent declining trend that has led to closure of fisheries, including First Nations subsistence fishing, throughout in the Canadian portion of the Yukon.   
...
* As in many other regions, the most recent index of run size is lower than the index of spawner abundance for the three Transboundary species that have both types of data (Chinook: `r round(sps_metrics$status[sps_metrics$region == "Transboundary" & sps_metrics$species == "Chinook" & sps_metrics$type == "Spawners"]*100, 1)`% compared to `r round(sps_metrics$status[sps_metrics$region == "Transboundary" & sps_metrics$species == "Chinook" & sps_metrics$type == "Run Size"]*100, 1)`%, coho: `r round(sps_metrics$status[sps_metrics$region == "Transboundary" & sps_metrics$species == "Coho" & sps_metrics$type == "Spawners"]*100, 1)`% compared to `r round(sps_metrics$status[sps_metrics$region == "Transboundary" & sps_metrics$species == "Coho" & sps_metrics$type == "Run Size"]*100, 1)`%, and sockeye: `r round(sps_metrics$status[sps_metrics$region == "Transboundary" & sps_metrics$species == "Sockeye" & sps_metrics$type == "Spawners"]*100, 1)`% compared to `r round(sps_metrics$status[sps_metrics$region == "Transboundary" & sps_metrics$species == "Sockeye" & sps_metrics$type == "Run Size"]*100, 1)`%), indicating that **declines in catch** have also occurred in recent years.
...
* All species for which we had reliable data have declined over the most recent three generations, with long-term trends either stable or declining.
* Unlike in other regions, where pink salmon have been doing relatively well, pink salmon have shown the most dramatic declines, and most recent spawner abundance was `r round(sps_metrics$status[sps_metrics$region == "Haida Gwaii" & sps_metrics$species == "Pink" & sps_metrics$type == "Spawners"]*100, 1)`% lower than the historical average.
...
* Chinook salmon have shown the biggest declines from historical average abundance (`r round(sps_metrics$status[sps_metrics$region == "Nass" & sps_metrics$species == "Chinook" & sps_metrics$type == "Spawners"]*100, 1)`%) with declines persisting in the most recent generation.
* Although sockeye run size is `r -round(sps_metrics$status[sps_metrics$region == "Nass" & sps_metrics$species == "Sockeye" & sps_metrics$type == "Run Size"]*100, 1)`% **below** historical average, increases over the last two years have put spawner abundance above historical average with an increasing short-term (3-gen) trend.
...
* All species are below historical average spawner abundance, with chum salmon having declined by `r -round(sps_metrics$status[sps_metrics$region == "Skeena" & sps_metrics$species == "Chum" & sps_metrics$type == "Spawners"]*100, 1)`%, with the exception of sockeye spawner abundance, which has recently increased to `r round(sps_metrics$status[sps_metrics$region == "Skeena" & sps_metrics$species == "Sockeye" & sps_metrics$type == "Spawners"]*100, 1)`% above historical average in 2022.
* Run size was only available for Skeena sockeye, and shows increases in the 1980s and 90s, with a decline from the peak in 1996. Slight increases over the last two years put current run size `r -round(sps_metrics$status[sps_metrics$region == "Skeena" & sps_metrics$species == "Sockeye" & sps_metrics$type == "Run Size"]*100, 1)`% below historical average, with stable long-term and 3-generation trends. Other research has found that the decline in sockeye abundance from the 1980s has been accompanied by erosion of diversity, with enhanced stocks increasingly dominating returns [@PriceEtAl2021PortfolioSimplificationArising].
...
* Despite the uptick in Fraser sockeye abundance in the early 2010s, the most recent spawner abundance and run size are below the devastating returns in 2009 that triggered a federal inquiry. Fraser sockeye spawner abundance in 2021 was `r -round(sps_metrics$status[sps_metrics$region == "Fraser" & sps_metrics$species == "Sockeye" & sps_metrics$type == "Spawners"]*100, 1)`% below historical average and in-season estimates for run size in 2022 was `r -round(sps_metrics$status[sps_metrics$region == "Fraser" & sps_metrics$species == "Sockeye" & sps_metrics$type == "Run Size"]*100, 1)`% below historical average.
* As for sockeye, Chinook spawner abundance is below historical average and run size is even further below historical average. However some life-history types are doing better than others [@AtlasEtAl2023TrendsChinookSalmon], highlighting the need to conserve the within-species diversity that makes salmon resilient to environmental change.
* Spawner abundance of chum, coho, and pink salmon are above historical average, **however** the these trends do not account for declining catch of the commercial fishing industry in this area. Total run size is below average for pink and coho, but the trend in the most recent three generations is stable providing some hope of recovery. Time series of chum salmon data should be interpreted with caution as they are based on expansion factors and spawner survey coverage has varied considerably through time. Run size is not (yet) available for chum salmon in the Fraser.
