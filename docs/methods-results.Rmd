...
Skeena sockeye escapement and run size were extracted from Pacific Salmon Commission reports [@tcnbCanadaNorthernBoundary2023,@SCSC2023]. For Skeena chum salmon, we used escapement to Area 4 reported in Table 32 of @tcnbCanadaNorthernBoundary2023, rather than expanding escapement from spawner surveys, because surveys of indicator stream have been incrasingly patchy over the last 30 years, making expansions uncertain.
...
Spawner abundance and run size were provided by DFO on data request (pers. comm. Marissa Glavas, Data Manager, Fraser River Stock Assessment) for interior Fraser coho CUs. We report run size as the sum of "Total Pre-Fishery Abundance" to all CUs and spawner abundance as the sum of "Total Return" (i.e., final spawner estimate plus fish removed from the system by DFO Salmon Enhancement Program or First Nations) to all CUs. We recognize that using these estimates ignores coastal coho populations, for which monitoring has been patchy.
...
3. **Short-term trend** over the most recent generation.

The first metric provides information on how the most recent spawner abundance compares to past values, while the second two metrics provide information on the direction of change and are less sensitive to the magnitude of current abundance.
...
The **short-term trend** is calculated as described above, but only using the time series over the most recent generation.
