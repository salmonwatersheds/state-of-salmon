...
date: "Version 2.3 (`r format(Sys.time(), '%Y-%b-%d')`)"
...
**2.3** (November 14, 2025) - Updated to include revised **total abundance** data for Fraser Chinook (all run timing groups now included, not just spring/summer; Chuck Parken pers. comm.) and recently published data for Columbia sockeye [@Ogden2025WildSalmonPolicy].
...
Escapement of Fraser Chinook is reported by the [PSC's Chinook Technical Committee (CTC) data sets](https://www.psc.org/publications/technical-reports/technical-committee-reports/chinook/ctc-data-sets/) and are described in @ChinookTechnicalCommittee2025AnnualReportCatch. These data were shared with us by DFO (Chuck Parken, pers. comm.), along with corresponding estimates of total run size for each of five run timing groups: Fraser Spring 4-2 (age 1.2), Fraser Spring 5-2 (age 1.3), Fraser Summer 5-2 (age 1.3), Fraser Summer 4-1 (age 0.3), and Fraser Fall 4-1 Harrison (age 0.3).
...
for(k in 1:2){
	...
	dat.k <- frck_cp %>% select(grep(c("esc", "run")[k], names(frck_cp)))
	names(dat.k) <- c("spr12", "spr13", "sum13", "sum03", "harrisonfall03", "sum")
	
	plot(frck_cp$year, dat.k$sum*10^-3, "n", bty = "l", las = 1, ylab = c("Spawner abundance (000s)", "Total abundance (000s)")[k], yaxs = "i", xlab = "", ylim = c(0, max(dat.k$sum*10^-3, na.rm = TRUE)))
	for(i in c(1:5)){
		if(i == 1){
			y1 <- rep(0, length(frck_cp$year))
			y2 <- data.frame(dat.k)[,1]
		} else if (i == 2){ 
			y1 <-  data.frame(dat.k)[,1:(i-1)]
			y2 <- apply(data.frame(dat.k)[,1:i], 1, sum)
		} else { 
			y1 <-  apply(data.frame(dat.k)[,1:(i-1)], 1, sum)
			y2 <- apply(data.frame(dat.k)[,1:i], 1, sum)
		}
		
		yr <- frck_cp$year
		if(i == 5){
			yr <- frck_cp$year[!is.na(frck_cp$harrisonfall03_esc)]
			y1 <- y1[!is.na(frck_cp$harrisonfall03_esc)]
			y2 <- y2[!is.na(frck_cp$harrisonfall03_esc)]
		}
		polygon(x = c(yr, rev(yr)),
						y = c(y1, rev(y2))*10^-3,
						col = col_frck[i], border = NA, xpd = NA)
	}
	points(frck_cp$year, dat.k$sum*10^-3, "o", pch = 21, bg = "white", xpd = NA)
	lines(frck_cp$year, genSmooth(
		abund =  dat.k$sum,
		years = frck_cp$year,
		genLength = 5
	)*10^-3, lwd = 1.5)
	# abline(h = seq(0, 800, 50), lty = 2, col =grey(0.8))
	if(k == 1) legend("topleft", fill = col_frck, border = NA, bty = "n", c("Spring 4-2", "Spring 5-2", "Summer 5-2", "Summer 4-1", "Fall 4-1 (Harrison)"))
	mtext(side = 3, line = 0.5, c("a) Escapement to five Fraser Chinook stocks", "c) Run size for five Fraser Chinook stocks")[k], adj = 0)
	
	# Proportion
	plot(frck_cp$year, frck_cp$esc_total, "n", bty = "l", las = 1, ylab = c("Proportion of spawner abundance", "Proportion of total abundance")[k], ylim = c(0, 1), yaxs = "i")
	for(i in c(1:5)){
		if(i == 1){
			y1 <- rep(0, length(frck_cp$year))
			y2 <- data.frame(dat.k)[,1]
		...
		polygon(x = c(frck_cp$year, rev(frck_cp$year)),
						y = c(y1/frck_cp$esc_total, rev(y2/frck_cp$esc_total)),
	...
	mtext(side = 3, line = 0.5, c("b) Proportional contribution of five stocks to aggregate escapement", "d) Proportional contribution of five stocks to aggregate run size")[k], adj = 0)
}
